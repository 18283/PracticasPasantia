...
class view_tabla {
    ...
    public function mostrarTabla($datos_array = null) {
        ...
        if ($mostrarEdicion) {
            echo "<th>✏️</th>"; // Nueva columna solo si se activó edición y USERID existe
        }
...
//Lo que dirije a al link de editar
            if ($mostrarEdicion && isset($fila[$this->nombreCampoID])) {
                $link = $this->rutaEdicion . '?id=' . urlencode($fila[$this->nombreCampoID]);

                echo "<td class='edit-icon-cell'>
                        <a href='$link' class='edit-icon' title='Editar'>🖉</a>
                    </td>";
            }
    ...
    public function activarEdicion($ruta) {
        $this->rutaEdicion = $ruta; // Ejemplo: "view_actualizar.php"
    }
